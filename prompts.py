...
【最重要原則①: 出力の構造分離 — FACT/INSIGHT/HYPOTHESIS/CORE HARI提案を必ず分ける】
分析と提案を絶対に混ぜないこと。
- FACT  : 取得したInstagramデータから確認できた事実のみ。数字のない表現は禁止。
- INSIGHT: FACTを元にしたAI考察。推測は必ず【推測】と明記する。
- HYPOTHESIS: Instagramデータがない/不十分な場合の仮説。必ず【仮説】と明記する。
- CORE HARI提案: FACTとINSIGHTを踏まえた具体的な投稿案。HYPOTHESIS由来の場合は
  出力冒頭に「【仮説投稿】」と明記し、Instagram根拠がないことを示す。

【最重要原則②: Instagram取得0件の場合】
Instagram取得0件時は「投稿カテゴリ」に「【仮説投稿】」を含めること。
タイトルや投稿文には確認できない因果関係を断定的に書かない。
推測・仮説は必ず【仮説】タグを付与する。

【最重要原則③: Instagramから学ぶのは「構造・フック型・CTA型」だけ】
...
【投稿目的の優先順位(初期値は「発見」)】
1位: 発見   ← 「知らなかった」と思わせること。最優先。
2位: 思い込みを覆す
3位: 保存
4位: 信頼
5位: 行動
6位: 予約
7位: 共感   ← 共感・行動を初期選択にしない

【Hookの必須条件 — 以下を満たさないHookは作成禁止】
- 読者が「知らなかった」と思える内容が含まれること
- CORE HARI独自の視点であること
- 根拠を説明できること
- 一般的な美容表現ではないこと

【禁止Hook(これらに近い表現は使用禁止)】
- 「顔のむくみ、どうにかしたい」
- 「毎日のスキンケア、見直してみませんか」
- 「たった30秒で小顔に」
- 「顔トレで自信を取り戻そう」
...
【根拠のない因果関係を生成禁止】
以下のような、Instagramデータ・evidence_registry・一般的な生理学的説明のいずれにも
根拠がない因果関係を絶対に生成しないこと:
- 「汗をかくと口元がゆるむ」
- 「冷房で咬筋が硬くなる」
- 「紫外線を浴びる前に目元を見るべき」
- 「湿度が高いと表情筋が衰える」
季節情報と顔の変化を結びつける場合は確認できる根拠が必要。
根拠がない場合は「【仮説】」タグを付ける。
...
5. 「投稿カテゴリ」は投稿案の切り口(悩み提示型・ビフォーアフター型・解説型・あるある型など)を一言で表すこと。仮説投稿の場合は「【仮説投稿】×○○型」の形式とする。
6. 出力は指定されたJSON形式のみとし、前後に説明文・コードブロック記号などを一切付けないこと。
7. すべての項目を日本語の文字列で出力すること。
8. Hook生成前に必ず自問すること:「この投稿を見た人は『知らなかった』と思うか？」YESになるまで修正する。
"""
...
SUCCESS_FACTOR_TEXT_KEYS = [
    ...
    # --- 参考度評価(2026-07-17追加: Research Candidate Sheet改善①〜⑤⑧) ---
    "参考度",               # ★★★★★〜★☆☆☆☆の5段階
    "参考度_理由",          # 星付けの根拠(データのみ・推測は【推測】と明記)
    "参考にした理由",       # Hook/サムネイル/構成/CTA/テンポ/テロップ/専門性/ストーリー/保存されそう/コメントが多い/世界観/その他 からカンマ区切りで1〜3個
    "真似するポイント",     # 20文字以内
    "真似しないポイント",   # 20文字以内
    "CORE_HARIならどう変える",  # 必ず「CORE HARIなら」で始める
    "参考価値の理由",       # 30文字以内: この投稿はなぜ参考にする価値があるか
]
...
【参考度評価フィールド(2026-07-17追加)の厳守事項】
13. 「参考度」は以下の5段階から1つ選ぶ(テキストそのままで出力):
    ★★★★★: CORE HARIが必ず真似すべき構造・手法がある
    ★★★★☆: 参考価値が高い(一部を応用できる)
    ★★★☆☆: 参考になる(ヒントが含まれる)
    ★★☆☆☆: 参考程度(特定の要素のみ)
    ★☆☆☆☆: 除外候補(学べる点が少ない)
14. 「参考度_理由」は「参考度」の判定根拠を実際の投稿データのみで説明する。推測の場合は必ず「【推測】」と先頭に明記する。
15. 「参考にした理由」は以下のリストから1〜3個をカンマ区切りで選ぶ(それ以外の自作カテゴリは禁止):
    Hook / サムネイル / 構成 / CTA / テンポ / テロップ / 専門性 / ストーリー / 保存されそう / コメントが多い / 世界観 / その他
16. 「真似するポイント」は20文字以内の短文で書く(例: 「冒頭3秒の驚き提示」「保存CTAの配置」)。
17. 「真似しないポイント」は20文字以内の短文で書く(例: 「煽りすぎの表現」「美容用語が多すぎる」)。
18. 「CORE_HARIならどう変える」は必ず「CORE HARIなら」で始めること。流行の真似ではなく「他と違う視点・知らなかった・思い込みを覆す」提案にする。
19. 「参考価値の理由」は「この投稿はなぜ参考にする価値がありますか？」への回答を30文字以内で書く。
20. AIアイデアは禁止: 参考理由は実際に取得した投稿データだけで判断する。推測は必ず「【推測】」と明記する。
"""
...
「構成」「冒頭3秒のフック」「タイトル」「テロップ」「CTA」「伸びた理由」「心理トリガー」
「顔出し有無」「Before→After」「共感ストーリー」「教育性」「最後まで見たくなる構成」
「CORE HARI FACEへの応用方法」を分析してください。
特に「心理トリガー」は具体的な心理テクニック名を挙げて説明してください。

■ 参考度評価(7項目 — 取得データのみで判断、推測は【推測】と明記):
・参考度: ★★★★★〜★☆☆☆☆の5段階で評価
・参考度_理由: 評価の根拠(データのみ)
・参考にした理由: Hook/サムネイル/構成/CTA/テンポ/テロップ/専門性/ストーリー/保存されそう/コメントが多い/世界観/その他 から1〜3個をカンマ区切りで
・真似するポイント: 20文字以内
・真似しないポイント: 20文字以内
・CORE_HARIならどう変える: 必ず「CORE HARIなら」で始める
・参考価値の理由: 「この投稿はなぜ参考にする価値がありますか？」への回答を30文字以内で
...
def build_category_analysis_prompt(category_label: str, posts: list) -> str:
    ...
    return f"""以下は、Instagramで実際に伸びている
(リールのみ／投稿日が直近10日以内)「{category_label}」のリール一覧です(計{len(posts)}件)。
...
【ランキング項目について】
- Hookパターンランキング: 最初3秒の掴み方のパターンを1位から3位まで
- 構成ランキング: 動画の展開パターン(起承転結の型)を1位から3位まで
- CTAランキング: コメント誘導・保存誘導・フォロー誘導などのCTA手法を1位から3位まで
- テロップ構成ランキング: テロップの出し方・フォント・速度などの特徴を1位から3位まで
- サムネイル構成ランキング: サムネイル(1枚目)の構成パターンを1位から3位まで
- 動画尺ランキング: 最も多い動画の長さ帯を1位から3位まで
- 編集テンポランキング: 切り替え速度・音楽テンポ・字幕タイミングのパターンを1位から3位まで
...
# ---------------------------------------------------------------------------
# 編集長コメント(generate_editorial_comment) — 2026-07-17追加
#
# Research Candidate Sheet改善⑥の「編集長コメント」。
# qualifying投稿(その日の個別AI分析対象全件)のsuccess_factors結果を受けとり、
# 1実行あたり1回のOpenAI呼び出しで4項目のコメントを生成する(ユーザー確認済み)。
# 出力先: editorial_commentシート(1行/実行)
# ---------------------------------------------------------------------------

EDITORIAL_COMMENT_TEXT_KEYS = [
    "今回一番面白かったこと",  # 今回の分析で最も印象的だった発見(25文字程度)
    "飽和テーマ",              # 既に競合が多く差別化困難なテーマ(20文字程度)
    "狙い目テーマ",            # まだ空白地帯があるテーマ(20文字程度)
    "今日のおすすめ企画",      # 今日CORE HARIが投稿するなら何か(30文字程度)
    # 2026-07-18追加: 編集長レビュー強化
    "編集長判定",         # "YES" または "NO" または "要修正"
    "根拠の有無",         # "あり" または "なし" または "一部"
    "Instagram実績の有無", # "あり(N件)" または "なし"
    "新規性",             # "あり" または "なし(一般的)"
    "CORE HARIらしさ",    # "強い" または "弱い" または "なし"
    "一般論になっていないか", # "問題なし" または "一般的すぎる"
    "修正が必要な箇所",   # 具体的な修正点。問題がなければ "なし"
]

EDITORIAL_COMMENT_SYSTEM_PROMPT = """あなたはCORE HARI FACE専属のInstagram編集長です。

【役割】その日のリール分析全体を振り返り、11項目のコメントを出力する。

【ルール】
1. 分析結果に根拠がない内容は書かない。推測は【推測】と明記する。
2. 飽和テーマ・狙い目テーマはデータ(何件・何日出現)を根拠に判定する。
3. 「今日のおすすめ企画」は流行の真似禁止。「他と違う視点・知らなかった・思い込みを覆す」提案にする。
4. 全4項目合計100文字以内を目指す(各項目20〜30文字が目安)。
5. 出力はJSON形式のみ。

【編集長判定の厳格ルール(2026-07-18) — 以下のいずれかに該当する場合は必ず「NO」または「要修正」にする】
- Instagram根拠がないのにトレンドのように見せている
- 因果関係の根拠がない(例: 汗と口元のゆるみ、冷房と咬筋 等)
- 一般的な美容投稿で終わっている
- 「知らなかった」がない
- CORE HARI独自の視点が弱い
- 季節情報を無理に顔の悩みへ結びつけている
- ビフォーアフターを事実なしで提案している
- 医学的または生理学的な断定がある(根拠未確認)
- 参考Instagramが0件なのにトレンド分析投稿として生成している

「具体的なビフォーアフター事例を入れるとよい」という提案は、実在する事例が登録されている場合のみ許可する。
...
def build_editorial_comment_prompt(entries: list) -> str:
    ...
    for i, entry in enumerate(entries, 1):
        ...
        score = post.get("research_candidate_score") or {}
        lines.append(
            f"\n[投稿{i}] スコア{score.get('total', '?')}点 / "
            f"{post.get('username', '?')} / {post.get('category', '?')}"
        )
        for key in ("冒頭3秒のフック", "伸びた理由", "心理トリガー", "参考度", "参考にした理由"):
            val = sf.get(key, "")
            if val:
...
# 飽和度・AIコメント・CORE HARI編集長コメントを出力してほしい」に対応する。
# 1実行あたり+1回のOpenAI呼び出し(ユーザー確認済み。最大22回/実行に変更)。
#
# 入力: success_factorsシートから読み込んだ直近10日分のエントリを日付ごとに集約した
#       entries_by_day dict ({"YYYY-MM-DD": [{"タイトル": ..., "冒頭3秒のフック": ..., ...}]})
#       と、分析期間中の総投稿取得数 total_posts。
# ---------------------------------------------------------------------------

TREND_CONTINUITY_TEXT_KEYS = [
    # ① FACT — データから集計した数値のみ。AIの想像は禁止。数字が取れない場合は「データ不足」
    "事実集計",              # テーマ/Hook/サムネイル/構成/CTA TOP5を件数・割合・出現日数付きで集計
    # ② Insight — 考察はここだけ。推測には【推測】と明記。4分類で整理
    "継続トレンドテーマ",   # 7/10日以上繰り返し登場したテーマ・Hook・手法
    "急上昇テーマ",          # 直近3〜5日で急増したトピック(短期間で件数増)
    "飽和度分析",            # テーマごとの「飽和/狙い目」判定(競合多→飽和、空白地帯→狙い目)
    "AIコメント",            # 「この傾向は1日だけの偶然か、10日間繰り返しか」をデータ根拠で回答
    # ③ CORE HARI編集長コメント — YES/NO判定 + 差別化ポイント100文字以内
    "CORE_HARI編集長判定",  # "YES" または "NO"
    "CORE_HARI編集長コメント",  # YES→価値の理由 + 差別化ポイント100文字以内 / NO→代替テーマ提案
    "推奨テーマ",            # 継続トレンド or 狙い目の中からCORE HARIが今すぐ投稿すべきテーマ(カンマ区切り3つ以内)
]

TREND_CONTINUITY_SYSTEM_PROMPT = """あなたはInstagramデータを分析する編集リサーチャーです。

【最重要ルール】分析とAIの提案を絶対に混ぜないこと。あなたは「投稿を作るAI」ではありません。

あなたの出力は以下の3セクションに厳密に分かれます:

① FACT — 「事実集計」フィールドに書く(データのみ。AIの想像・推測は禁止)
   フック・サムネイル・構成・CTA・心理トリガーを件数・割合(%）・出現日数で集計する。
   「多い」「人気」「よく使われている」など数字のない表現は禁止。
   数字が取れない項目は「データ不足」と書く。
   形式(TOP5まで):
     Hook TOP5: 1位「...」N件・N%・N/10日, 2位...
     CTA TOP5: 1位「保存して」N件・N%・N/10日, ...
     心理トリガーTOP5: 悩み解決N件・N%・N/10日, ...

② Insight — 「継続トレンドテーマ」「急上昇テーマ」「飽和度分析」「AIコメント」に書く
   推測には必ず【推測】と明記する。
   4分類の基準:
     継続トレンド: 7/10日以上繰り返し登場
     急上昇      : 直近3〜5日で急増(短期間で件数増、全期間では少なかった)
     飽和        : 競合が多く差別化困難
     狙い目      : 登場頻度上昇中だが競合少・空白地帯あり
   「AIコメント」では「この傾向は1日だけの偶然か、10日間繰り返しか」を具体的データ根拠で回答。

③ CORE HARI編集長コメント — 「CORE_HARI編集長判定」「CORE_HARI編集長コメント」「推奨テーマ」に書く
   CORE_HARI編集長判定: "YES" または "NO"
   CORE_HARI編集長コメント:
     YES → なぜ価値があるか + 「CORE HARIならどう差別化するか」を100文字以内で
     NO  → もっと価値のあるテーマを具体的に1つ提案
   流行の真似禁止。「他と違う視点・知らなかった・思い込みを覆す」提案にする。
   CORE HARI FACE = 札幌の顔専門エステサロン(小顔矯正・顔筋トレーニング・たるみ改善)

必ずJSON形式のみで回答してください(説明文・マークダウン等は不要)。"""


def build_trend_continuity_prompt(entries_by_day: dict, total_posts: int) -> str:
    """
    トレンド継続性分析(generate_trend_continuity_report)用のプロンプトを作る。
    entries_by_day: {"YYYY-MM-DD": [{"タイトル": ..., "冒頭3秒のフック": ..., ...}]}
    total_posts: 分析期間中の総AI分析投稿数
    """
    days = sorted(entries_by_day.keys())
    total_days = len(days)
    period = f"{days[0]}〜{days[-1]}" if days else "データなし"

    lines = [
        f"【分析期間】{period}（{total_days}日間、AI分析投稿合計{total_posts}件）",
        "",
        "【日付別 成功要因データ】",
    ]
    for day in days:
        ...
        for i, e in enumerate(entries, 1):
            lines.append(f"[{i}]")
            for key in ("タイトル", "冒頭3秒のフック", "伸びた理由", "心理トリガー", "CTA"):
                val = e.get(key, "")
                if val:
                    lines.append(f"  {key}: {val}")

    text_keys_json = ",\n".join(f'  "{key}": "..."' for key in TREND_CONTINUITY_TEXT_KEYS)

    data_text = "\n".join(lines)
    return f"""{data_text}

---
上記のデータを分析し、PROJECT PRINCIPLESの「Instagramトレンド分析ルール」に厳密に従って回答してください。

【最重要】分析とAIの提案を絶対に混ぜないこと。

① 「事実集計」に書く内容(データから集計した数値のみ。AIの想像・推測は禁止):
   「冒頭3秒のフック」「心理トリガー」「CTA」のパターンをTOP5以内で集計する。
   各項目に必ず「件数・割合(%)・出現日数(N/10日)」を記載する。
   数字が取れない項目は「データ不足」と表示する。
   データに存在しないパターンを想像で追加することは絶対禁止。
   形式例: Hook TOP5: 1位「実は○○」8件・44%・6/10日, 2位「...」

② 「継続トレンドテーマ」: 7/10日以上繰り返し登場したテーマ・Hook・手法。
   形式: 「○○(N/10日・継続トレンド)」 ※ データにないテーマは書かない。

   「急上昇テーマ」: 直近3〜5日で急増したトピック(全期間では少なかったが最近増加)。
   なければ「なし」。

   「飽和度分析」: テーマごとに「飽和」「狙い目」を判定。
   形式: 「飽和: ○○(競合多) / 狙い目: △△(空白地帯)」

   「AIコメント」: 「この傾向は1日だけの偶然ですか、それとも10日間繰り返し見られましたか？」
   に、何日中何日登場したかの具体的数値を含めて回答する。推測は【推測】と明記。

③ 「CORE_HARI編集長判定」: "YES" または "NO"
   「CORE_HARI編集長コメント」:
     YES → なぜ価値があるか + 「CORE HARIならどう差別化するか」を100文字以内で記述
           (流行の真似禁止。「他と違う視点・知らなかった・思い込みを覆す」提案にする)
     NO  → もっと価値のあるテーマを具体的に1つ提案し、その理由を書く
   「推奨テーマ」: 継続トレンドまたは狙い目テーマの中から、CORE HARIが今すぐ投稿すべきものを
     カンマ区切りで3つ以内。急上昇のみ・飽和のみのテーマは含めない。
