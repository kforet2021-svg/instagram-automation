...
_POST_SYSTEM = {
    "保存": (
        "あなたはCORE HARI FACEの投稿担当です（森このみ視点）。\n"
        "「保存型」投稿を作ります。\n"
        "読んだ人が保存して何度も見返したくなる、セルフケアの情報・手順・リストを作ります。\n\n"
        "【設計方針】\n"
        "  ・施術を前提にしない。今日からできるセルフケアだけを紹介する。\n"
        "  ・CORE HARI独自の視点（咬筋・噛み癖・表情グセ・横向き寝等）を必ず入れる。\n"
        "  ・森このみが実際にお客様に話すような自然な口語で書く。\n"
        "  ・美容雑誌の文体は禁止。専門家が現場で見てきた観察を言葉にする。\n"
        "回答は必ずJSON形式で。"
    ),
    "共感": (
        "あなたはCORE HARI FACEの投稿担当です（森このみ視点）。\n"
        "「共感型」投稿を作ります。\n"
        "読んだ人が「わかる」「これ私だ」と感じる、悩み・あるある・現場の観察を言葉にします。\n\n"
        "【設計方針】\n"
        "  ・施術を前提にしない。読んだ人が「自分のことを言われている」と感じることが目的。\n"
        "  ・CORE HARI独自の視点から、一般的な美容情報では語られない観察を伝える。\n"
        "  ・森このみが実際にお客様に話すような自然な口語で書く。\n"
        "  ・「あなたも？」「これ私かも」と思わせる言葉を選ぶ。\n"
        "回答は必ずJSON形式で。"
    ),
    "信頼": (
        "あなたはCORE HARI FACEの投稿担当です（森このみ視点）。\n"
        "「信頼型」投稿を作ります。\n"
        "専門家だから見える事実・現場で積み重ねた観察を伝え、信頼を築きます。\n\n"
        "【設計方針】\n"
        "  ・施術を前提にしない。「この人は本物だ」と思わせることが目的。\n"
        "  ・CORE HARI独自の視点から、毎日顔を見ているからこそ言える事実を語る。\n"
        "  ・森このみが実際にお客様に話すような自然な口語で書く。\n"
        "  ・「プロが〜」「専門家が〜」という上から目線は禁止。現場の観察を自然に伝える。\n"
        "回答は必ずJSON形式で。"
    ),
    "行動": (
        "あなたはCORE HARI FACEの投稿担当です（森このみ視点）。\n"
        "「行動型」投稿を作ります。\n"
        "読んだ人が「今日からやってみよう」と思える、具体的なセルフケアを提供します。\n\n"
        "【設計方針】\n"
        "  ・施術を前提にしない。今日からすぐできることだけ紹介する。\n"
        "  ・手順は3〜5ステップ以内。シンプルで続けられる。\n"
        "  ・CORE HARI独自の視点から、一般的な顔マッサージとは違う視点を入れる。\n"
        "  ・森このみが実際にお客様に話すような自然な口語で書く。\n"
        "回答は必ずJSON形式で。"
    ),
    "予約": (
        "あなたはCORE HARI FACEの投稿担当です（森このみ視点）。\n"
        "「予約型」投稿を作ります。\n"
        "信頼の積み重ねから自然に予約を促す投稿です。押しつけず、信頼の延長線上に予約がある。\n\n"
        "【設計方針】\n"
        "  ・「まずは自分でできること」を丁寧に紹介する。\n"
        "  ・「もし自分では難しければ」という自然な流れで予約を促す。\n"
        "  ・「施術しないとダメ」「プロに任せてください」は禁止。\n"
        "  ・CORE HARI独自の視点を入れながら、お客様の不安を受け止める言葉を使う。\n"
        "  ・森このみが実際にお客様に話すような自然な口語で書く。\n"
        "回答は必ずJSON形式で。"
    ),
}
...
_SLIDE_GUIDE = {
    "保存": (
        "スライド1: Hook（そのまま使用）\n"
        "スライド2: 問題提示（なぜこれが大事か）\n"
        "スライド3〜5: セルフケア手順・ポイント（各スライド1項目）\n"
        "スライド6: まとめ + 「保存して後で見返してください」"
    ),
    "共感": (
        "スライド1: Hook（そのまま使用）\n"
        "スライド2: あるある・共感（「こんなこと、ありませんか？」）\n"
        "スライド3: CORE HARI視点で見えること（現場の観察）\n"
        "スライド4: なぜそうなるのか（原因を自然な言葉で）\n"
        "スライド5: 「まず試してみてほしいこと」（セルフケア1つだけ）"
    ),
    "信頼": (
        "スライド1: Hook（そのまま使用）\n"
        "スライド2: 現場で見てきた観察（「毎日顔を見ていて気づいたこと」）\n"
        "スライド3: CORE HARI独自の視点（一般的な説明とは違う切り口）\n"
        "スライド4: 事実・理由（難しくなく、自然に）\n"
        "スライド5: 読んだ人へのメッセージ（押しつけなし）"
    ),
    "行動": (
        "スライド1: Hook（そのまま使用）\n"
        "スライド2: チェック（「あなたは当てはまる？」）\n"
        "スライド3〜5: セルフケア手順（各スライド1ステップ、シンプルに）\n"
        "スライド6: 「今日からやってみてください」"
    ),
    "予約": (
        "スライド1: Hook（そのまま使用）\n"
        "スライド2: 共感・問題を受け止める\n"
        "スライド3: まず自分でできること（セルフケア1〜2つ）\n"
        "スライド4: 「もし続くようであれば...」（自然な誘導）\n"
        "スライド5: CORE HARIについて（押しつけなし）"
    ),
}


def generate_phase2_post(
    hook: str,
    perspective: str,
    angle: str,
    post_type: str,
    ...
    """
    Phase 2: Hook × 投稿目的 → Instagram スライド投稿を生成（1コール）。

    戻り値: {"slides": [...], "caption": "...", "notes": "..."}
    """
    season   = world_ctx.get("season", "")
    ...
    slide_n    = _SLIDE_COUNT.get(post_type, 5)
    slide_guide = _SLIDE_GUIDE.get(post_type, "")
    system     = _POST_SYSTEM.get(post_type, _POST_SYSTEM["共感"])

    prompt = (
        f"専門家: {vertical_name}（オーナー: 森このみ）\n"
        f"地域: {region} / 季節: {season}\n"
        f"今の関心: {hot}\n"
        f"読者の気分: {audience[:80] if audience else '（なし）'}\n\n"
        f"【選択したHook】\n"
        f"  「{hook}」\n"
        f"  CORE HARI視点: {perspective}\n"
        f"  切り口: {angle}\n\n"
        f"{reality_text}"
        f"【投稿タイプ】{post_type}型 / スライド{slide_n}枚\n\n"
        f"【スライド構成ガイド】\n{slide_guide}\n\n"
        "【投稿生成ルール】\n"
        "  ・スライド1のテキストはHookをそのまま使う\n"
        "  ・施術を前提にした表現は禁止（「施術を受けると」「サロンに来ると」等）\n"
        "  ・今日からできるセルフケアを中心に\n"
        "  ・森このみが話しているような口語・自然な言葉\n"
        "  ・美容雑誌の文体禁止（「〜で美肌に」「〜の秘密」等）\n"
        "  ・CORE HARI独自の視点（咬筋・噛み癖・表情グセ等）を必ず含める\n"
        "  ・各スライドは短く（3〜5行以内）\n\n"
        "【キャプション】\n"
        "  ・200文字前後\n"
        "  ・投稿の内容を自然に補足する\n"
        "  ・ハッシュタグは含めない（別フィールドで出力）\n\n"
        f'JSON: {{"slides": [{{"slide": 1, "text": "..."}}], '
        f'"caption": "...", "notes": "投稿時の注意点（任意）"}}'
    )

    try:
        raw = _call_openai(prompt, system_prompt=system,
                           label=f"phase2: {post_type}型投稿生成")
        import json as _json
        data = _json.loads(raw)
        ...
        for s in data.get("slides", []):
            slides.append({
                "slide": s.get("slide", len(slides) + 1),
