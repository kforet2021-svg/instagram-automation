...
_TOPIC_CANDIDATES_SYSTEM = (
    ...
    "【Hook生成の手順 — 必ずこの順序で行う】\n"
    "  Step1. Topic（伝えたい事実・悩み・視点）を決める\n"
    "  Step2. 切り口を選ぶ（下記リストから1つ選ぶ）\n"
    "  Step3. 切り口 × Topic を掛け合わせてHook文を作る\n\n"
    "【切り口リスト】\n"
    "  ・勘違い   … 「それ、勘違いです」「実はみんな間違えている」\n"
    "  ・実は     … 「実は○○が原因でした」「実は逆効果です」\n"
    "  ・専門家視点 … 「施術中だけが知っている」「プロが最初に見る場所」\n"
    "  ・先にして … 「先に○○してください」「〇〇する前に読んでください」\n"
    "  ・NG行動  … 「やってはいけない○○」「それ、悪化させてます」\n"
    "  ・比較    … 「○○と△△の違い」「やっている人・やっていない人」\n"
    "  ・チェック … 「今すぐ確認してください」「当てはまったら要注意」\n"
    "  ・〜な人へ … 「○○と思っている人へ」「○○が気になる人へ」\n"
    "  ・共通点  … 「〜な人には共通点があります」\n"
    "  ・ランキング … 「多い順に」「よく聞く悩みTOP3」\n\n"
    "【Hook の条件】\n"
    "  ・続きを見たい・読みたいと思わせること（最優先）\n"
    "  ・Instagram 1枚目 / Threads 1行目として使える一文\n"
    "  ・15〜30文字程度\n"
    "  ・お客様が実際に使う言葉（専門用語・雑誌語禁止）\n\n"
    "  NG（単なる質問 — 禁止）:\n"
    "    ✗ 「顔の疲れを感じていますか？」\n"
    "    ✗ 「表情筋をケアしていますか？」\n\n"
    "  OK（続きを読みたくなる）:\n"
...
def generate_topic_candidates_ai(
    ...
    prompt = (
        ...
        f"{obs_text}"
        "【生成手順 — 必ずこの順序】\n"
        "  Step1. ブランド領域（顔・表情筋・むくみ・たるみ等）のTopicを10個想定する\n"
        ...
        "  angle     : 使った切り口（上記10種から1つ）\n"
        "  theme     : 内部テーマ識別子（10文字以内）\n"
        "  post_type : 保存/共感/信頼/行動/Threads から1つ\n"
        "  reason    : 選定理由（1〜2行）\n\n"
        'JSON: {"candidates": [{"hook":"...","angle":"...","theme":"...","post_type":"保存","reason":"..."}, ...]}'
    )
